...
//LEFT JOIN masterrental AS MR ON TU.UserID = MR.ID
     ...
     //         }
           ...
           $sql2 = "SELECT MT.* ,MR.Title, MR.FName, MR.LName FROM  {$db} as MT LEFT JOIN masterrental as MR ON MT.Rental = MR.ID WHERE MT.Floor = '{$_POST['Floor']}' AND Year = '{$year1}' AND Room IN (SELECT Room FROM masterroom WHERE STATUS = 1) ORDER BY MT.Room ASC";
      //  echo $sql2;
...
//             }
         ...
         //echo $sql;
           ...
           //    var_dump($row);
            ...
            <tr  id = "<?php echo  $row['ID']; ?>"><input id = "ids" name= "id[]" value="<?= $row['ID']; ?>" type="hidden" >
           ...
           <td align = "center"><input id="endwater" name="endwater[]" type="text" size="10"   value="<?= $row['EndWater']; ?>"></td>
            ...
            <td align = "center"><input id="etc" name="etc[]" type="text" size="10"   value="<?= $row['Etc']; ?>"></td>
                <td align = "center">    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#myModal" data-record-title="<?=  $row['Room']; ?>"  value = "<?php echo "id=".$row['ID']."&month=".$_POST['month']."&year=".$year1;?>"> ลบข้อมูล</button>
              </td>
            </tr>
